...
public class BaseAutomationTest {
	...
	protected synchronized void quitDriver(WEB_DRIVER webDriver) {
		logger.info("Starting of method quitDriver in Sastodeal Test ");
		...
		try {
			if (driver != null) {
				...
				logger.debug(webDriver + " Web driver quit successfully in BaseSastodealTest ");
			}
		} catch (Exception ex) {
			logger.error(ex.getMessage());
			driver = null;
		}
		logger.info("Ending of method quitDriver in Sastodeal");
	}

	@AfterSuite
	protected synchronized void quitAllDrivers() {
		logger.info("Starting of method quitAllDrivers in BaseSastodealTest");
					...
					synchronized (webDriverPool) {
						...
						try {
							...
							if (driver != null) {
								...
								Thread.sleep(3000);
								logger.debug("Driver quit successfully in BaseSastodealTest ");
							}
						} catch (Exception ex) {
							logger.error(ex.getMessage());
							driver = null;
		...
		logger.info("Ending of method quitAllDrivers in BaseSastodealTest");
	}
